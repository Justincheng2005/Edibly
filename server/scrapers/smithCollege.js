...
async function startScraper() {
    ...
    try {
        ...
        try {
            ...
            // Process both breakfast and lunch menus
            const menuTypes = [
            ...
            for (const menuType of menuTypes) {
                try {
                    console.log(`\nProcessing ${menuType.name} menu`);

                    // If going back to lunch menu, we need to make sure we're in the right location
                    if (menuType.name === "Lunch") {
                        try {
                            // First try to press back button if it exists
                            try {
                                const backButtons = await driver.findElements(By.css('#cbo_nn_backToUnitBtn, .cbo_nn_backButton, button.btn'));
                                if (backButtons.length > 0) {
                                    await backButtons[0].click();
                                    console.log("Clicked back button");
                                    await driver.sleep(1000);
                                } else {
                                    // If no back button, try to navigate directly
                                    await driver.get('https://cbweb.smith.edu/NetNutrition/1#');
                                    console.log("Navigated back to main page");
                                    await driver.sleep(2000);
                                }
                            } catch (backError) {
                                console.error(`Error clicking back button: ${backError.message}`);
                                // If clicking fails, try direct navigation
                                await driver.get('https://cbweb.smith.edu/NetNutrition/1#');
                                console.log("Navigated back to main page after error");
                                await driver.sleep(2000);
                            }

                            // Click on Chapin Hall again
                            const chapinLinkAgain = await driver.findElement(By.css('.cbo_nn_unitImageLink[onclick*="unitsSelectUnit(1)"]'));
                            await chapinLinkAgain.click();
                            console.log("Clicked on Chapin Hall again for Lunch menu");
                            await driver.sleep(1000);
                            ...
                            for (let i = 0; i < allMenuLinks.length; i++) {
                                ...
                                // If this is the lunch menu with text "LUNCH", update the selector
                                if (menuText.toUpperCase().includes('LUNCH')) {
                                    menuType.selector = `.cbo_nn_menuLink[onclick="${menuOnclick}"]`;
                                    console.log(`Updated lunch selector to: ${menuType.selector}`);
                                }
                            }
                        } catch (navigationError) {
                            console.error(`Error navigating back for ${menuType.name}:`, navigationError);
                        }
                    }

                    // Click on menu (Breakfast or Lunch)
                    const menuLink = await driver.findElement(By.css(menuType.selector));
                    await menuLink.click();
                    console.log(`Clicked on ${menuType.name} menu`);
                    ...
                    console.log(`Found ${foodItems.length} food items in ${menuType.name} menu`);
                    ...
                    for (let i = 0; i < foodItems.length; i++) {
                        try {
                            ...
                            try {
                                ...
                                // Navigate back to the current menu
                                const chapinLink = await driver.findElement(By.css('.cbo_nn_unitImageLink[onclick*="unitsSelectUnit(1)"]'));
                                await chapinLink.click();
                                console.log("Clicked on Chapin Hall again");
                                await driver.sleep(1000);

                                const menuLinkAgain = await driver.findElement(By.css(menuType.selector));
                                await menuLinkAgain.click();
                                console.log(`Clicked on ${menuType.name} menu again`);
                                await driver.sleep(1000);
                            } catch (recoveryError) {
                    ...
                    console.log(`\nFinished processing ${menuType.name} menu`);
                } catch (menuError) {
